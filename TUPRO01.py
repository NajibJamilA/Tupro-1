# Melakukan Import Library Random, Serta import library Math untuk operasi sin dan cos
import random
from math import sin, cos

# Membuat array untuk data limit x dan limit y
x_limit = [-5, 5]
y_limit = [-5, 5]

# Fungsi rumus yang akan dicari nilai minimumnya
def function(x,y):
    rumus = (cos(x) + sin(y))**2 / (x**2 + y**2)
    return 1 / (0.1 + rumus)

# Fungsi untuk pembuatan populasi dan kromosomnya
def generate_population(p,k):
    return [[random.randint(0,9) for _ in range(k)] for _ in range(p)]

# Fungsi split/membagi kromosom menjadi 2 bagian
...
def decode(kromosom, limit) :
    ...
    for i in range(len(kromosom)):
        ...
        kali += num * (10**-(i+1))
        bagi += 9 * (10**-(i+1))
    return limit[0] + (((limit[1] - limit[0]) / bagi) * kali)
...
def elitisme(population, best_kromosom_generation, bad_kromosom, total_fitness):
    if  best_kromosom_generation[1] > bad_kromosom[0] and (best_kromosom_generation[0] not in population):
        ...
        print('\nProses Elitisme')
        print(f'Kromosom Ke -{bad_kromosom[2]+1}: {bad_kromosom[1]}, nilai fitness: {bad_kromosom[0]}')
        print(f'ubah menjadi {best_kromosom_generation[0]}, nilai fitness: {best_kromosom_generation[1]}\n')
    return population, total_fitness

# Fungsi untuk menentukan kromosom terbaik
def best_kromosom_seleksi(population):
    max_fitness = -999
...
for gen in range(generasi):
      ...
      best_kromosom = best_kromosom_seleksi(population)
      ...
      if  gen != generasi-1 :
          for i in range(population_total // 2):
              ...
              child_1, child_2 = mutasi(childs[0], childs[1])
...
# Memanggil fungsi dan menampilkan kromosom terbaik pada keseluruhan generasi
print('\n=====================================================')
...
print('Nilai Fitness            = ', most_best[1])
